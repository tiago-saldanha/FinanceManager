...
public class ResendEmailService(HttpClient httpClient, IConfiguration configuration) : IEmailService
{
    ...
    public async Task SendPasswordResetAsync(string toEmail, string resetLink)
    {
        ...
        var body = new
        {
            ...
            to = new[] { toEmail },
            subject = "RedefiniÃ§Ã£o de senha â€” Finance Manager",
    ...
    private static async Task<string> BuildEmailHtmlAsync(string resetLink)
    {
        ...
        var resourceName = "Finanza.Infrastructure.Templates.password-reset.html";

        await using var stream = assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException($"Template '{resourceName}' nÃ£o encontrado.");
